...
- `python -m src.coder` starts the interactive CLI
- `src/coder/cli/app.py` handles commands and exit-time memory jobs
- `src/core/product/conversation/manager.py` orchestrates one user turn
- `src/core/agent.py` runs the ReAct graph
- `tools/mcp_servers/freeweb/` is an optional submodule for local web tooling; use `git submodule update --init --recursive` after cloning if it is absent
...
- `src/coder/` provides the terminal application
- `src/core/` provides the agent runtime, tool registry, memory stores, RAG, prompt builders, and MCP client/server code
...
Code: `src/core/agent.py`, `src/langgraph/graph.py`

The agent is written in the LangGraph mental model: a state object moves through named nodes and conditional edges. The repository also contains a small local `src/langgraph/` compatibility implementation with `StateGraph`, `START`, and `END`, so the ReAct control flow remains easy to inspect.
...
Code: `src/langchain_core/messages.py`
...
Code: `src/core/protocols/mcp/client.py`, `src/core/protocols/mcp/server.py`
...
Code: `src/core/rag.py` and `src/core/memory_pipeline.py`

- Reads `.md`, `.txt`, `.csv`, `.tsv`, `.docx`, `.pdf`, and `.xlsx` files from `data/knowledge_base/`
...
Code: `src/core/tools.py`, `src/core/providers/`, `src/core/protocols/mcp/`
...
| Tool | Source | Purpose |
| --- | --- | --- |
| `Calculator` | `src/core/tools.py` | Safe arithmetic using Python AST parsing |
| `Now` | `src/core/tools.py` | Current local date and time |
...
- Prefer local built submodule: `node tools/mcp_servers/freeweb/dist/index.js`
...
- `src/core/` is the low-level runtime and compatibility layer
- `src/core/product/` is the more product-facing wrapper layer
- `src/core/providers/` packages external capabilities into providers
- `src/core/protocols/mcp/` implements the JSON-line MCP client/server
...
- Python test cases for core logic
- Verifies MCP tooling and Agent workflows
...
- `python -m src.coder memory-worker <job.json>` - run a memory consolidation job
- `python -m src.coder mcp-server amap` - serve Amap MCP tools
- `python -m src.coder mcp-server weather` - serve weather MCP tools
- `git submodule update --init --recursive` - fetch the optional `tools/mcp_servers/freeweb/` web tooling submodule
...
- Prefer `src/core/product/` for high-level imports
